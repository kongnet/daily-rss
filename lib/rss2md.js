...
async function getBingOld () {
  ...
  if (item) {
    ...
    return {
      ...
      pic: descToUrl(item?.description?._text || item?.description?._cdata)
    }
  }
  return {}
}
async function getBing () {
  const res = await fetchSite(
    `https://bing.ee123.net/img/?date=${new Date().date8()}&size=1920x1080&type=json`
  )

  const { imgurl, imgtitle } = await res.json()
...
async function renderMD (data) {
  ...
  setSideBar(files, monthDir)
}

function setSideBar (files, monthDir) {
  let result = []
  ...
  arr.forEach(a => {
    if (a < 200000) return
    const monStr = `${Math.floor(a / 100)}-${
      a % 100 < 10 ? '0' + (a % 100) : a % 100
    }`
  ...
  hFs.saveFile(monthDir + '/_sidebar.md', sidebarContent)
  hFs.saveFile(monthDir + '/_navbar.md', '')
  hFs.saveFile(monthDir + '/../../today-html/_sidebar.md', sidebarContent)
  hFs.saveFile(sidebarPath, sidebarContent)
}
...
async function genMD (obj) {
  ...
  for (const source in obj) {
    result.push(`## ${obj[source]['alias'] ? obj[source]['alias'] : source}`)
